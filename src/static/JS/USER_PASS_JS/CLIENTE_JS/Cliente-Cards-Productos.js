...
function handleButtonSelection(selectedButton) {
    document.querySelectorAll('.btn-group button').forEach(btn => {
        btn.classList.toggle('selected-button', btn === selectedButton);
        btn.classList.toggle('disabled-button', btn !== selectedButton);
        btn.disabled = btn !== selectedButton;
    });

    // Verifica si se seleccionó LASER FACTURA o RESAL FACTURA
    if (selectedButton.id === "laser-factura-btn" || selectedButton.id === "resal-factura-btn") {
        toggleIVAVisibility(selectedButton.id); // Solo para LASER FACTURA y RESAL FACTURA
        actualizarIVA(); // Actualiza el IVA solo si es necesario
    } else {
        toggleIVAVisibility(null); // Si no es LASER FACTURA o RESAL FACTURA, ocultar el IVA
        actualizarCostoFinal(); // Si no hay IVA, actualizar el costo final sin IVA
    }
}



// Inicializa la funcionalidad de botones
function initializeButtons() {
    document.querySelectorAll('.btn-group button').forEach(button => {
        button.addEventListener('click', function () {
            if (!button.disabled) handleButtonSelection(this);
        });
    });
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const buttonEndpoints = {
        "laser-factura-btn": "/Clientes_rutas/Cliente_cards_laser_factura",
        "laser-remision-btn": "/Clientes_rutas/Cliente_cards_laser_remision",
        "resal-factura-btn": "/Clientes_rutas/Cliente_cards_resal_factura",
        "resal-remision-btn": "/Clientes_rutas/Cliente_cards_resal_remision",
    };

    Object.keys(buttonEndpoints).forEach(buttonId => {
        const button = document.getElementById(buttonId);
        if (button) {
            button.addEventListener('click', function () {
                currentEndpoint = buttonEndpoints[buttonId]; // Asignar el endpoint a la variable
                fetchProducts(currentEndpoint);  // Llamar a la función fetchProducts con el endpoint correcto
            });
        }
    });
});
